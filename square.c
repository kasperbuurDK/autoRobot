...
/* added by me/kasper
 * Exercise 3.1 adding a struct and an array with many of this struct
 */
...
*/
    ...
    for (int i = 0; i < logCounter; i++) {
        logFile dataSample = logArray[i];
        //fprintf(fp, "%d,%d,%0.8f,%0.8f\n", i, dataSample.time, dataSample.speedL, dataSample.speedR);
        fprintf(fp, "%i,%i,%0.8f,%0.8f,\n", i, dataSample.time, dataSample.motorSpeedL, dataSample.motorSpeedR);
    }
    fclose(fp);
...
void update_motcon(motiontype *p){
    ...
    switch (p->curcmd){
        ...
        case mot_turn:                 // fixed the turning problem by making the other wheel turn backwards
            if (p->angle>0){
                if (p->right_pos-p->startpos < (p->angle*p->w)/2)    // only need to rotate half the distance as before
                {
                    p->motorspeed_r=p->speedcmd/2;                  //to maintain same speed, we divide by 2
